...
class CDQL:
    def __init__(self, use_gpu=False, region_num=48, target_dist="default_gamma"):
        self.gamma = 0.9
        ...
        self.sim_controller = ABP("temp.h5", region_num, target_dist)
        self.buffer = ReplayBuffer(1e6)
        self.batch_size = 32
        self.all_actions = [0.0, 0.25, 0.5, 0.75, 1.0, 1.25, 1.5]
    ...
    def train(self, num_decisions=350):
        ...
        for i in range(90):
            ...
            tag = "_train_init"
            self.sim_controller.run_decorrelation(20, tag)
            ...
            for j in range(num_decisions):
                ...
                try:
                    self.sim_controller.update_activity(actions, tag)
                    system_states, system_rewards, system_states_cluster = self.sim_controller.run_step(
                        is_detailed=True, more_detailed=False, tag=tag)
                ...
                transition_to_add.extend([reward, state])
                ...
                if (j % 50 == 49):
                    ...
                    np.save(episode_folder_name + "rb_episode_samples",
                            np.array(rb_episode_samples))
    ...
    def test(self, num_decisions=1000):
        ...
        for i in range(num_decisions):
            ...
            # The "System States" are recorded every 0.25 seconds(only if more_detailed=True). Excludes 0th second
            all_system_states.append(system_states)
            ...
            if (i % 10 == 9):
                ...
                np.save(self.folder_name + "grid_states",
                        np.array(all_grid_states))
                np.save(self.folder_name + "grid_states_cluster",
                        np.array(all_grid_states_cluster))
